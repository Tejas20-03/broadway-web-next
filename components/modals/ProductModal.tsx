  ...
  return (
    <div className="fixed inset-0 z-[200] flex sm:items-center justify-center sm:p-4">
      ...
      <div className="relative bg-white dark:bg-[#0a0a0a] w-full h-[100dvh] sm:h-[85vh] sm:max-w-6xl sm:rounded-[2.5rem] flex flex-col md:flex-row shadow-2xl animate-slide-in border-none sm:border sm:border-neutral-200 dark:border-white/10 overflow-hidden">
        ...
        <div className="hidden md:block relative w-[40%] bg-neutral-100 dark:bg-[#121212] shrink-0 group h-full">
          ...
          <button onClick={onClose} className="absolute top-4 left-4 bg-white/80 dark:bg-black/40 hover:bg-white dark:hover:bg-black/80 text-neutral-900 dark:text-white p-3 rounded-full backdrop-blur transition-all border border-neutral-200 dark:border-white/10 z-20">
            <X size={24} />
          </button>
        ...
        {/* RIGHT PANEL — options */}
        <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-[#0a0a0a] relative overflow-hidden h-full">
                ...
                {[1, 2, 3].map(i => (
                  <div key={i} className="space-y-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="h-4 bg-neutral-200 dark:bg-neutral-800 rounded animate-pulse w-36" />
                    </div>
                    <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                      {[1, 2, 3, 4, 5, 6].map(j => (
                        <div key={j} className="aspect-square bg-neutral-200 dark:bg-neutral-800 rounded-2xl animate-pulse" />
                      ))}
                    </div>
                  </div>
                ))}
                ...
                {resolvedProduct.sizes && resolvedProduct.sizes.length > 0 && !isSingleOrDashSize && (
                  <section>
                    <div className="flex items-center gap-2 mb-4">
                      <span className="w-5 h-5 md:w-6 md:h-6 rounded-full bg-yellow-500 text-neutral-900 dark:text-white flex items-center justify-center font-bold text-[10px] md:text-xs">1</span>
                      <h3 className="text-base md:text-lg font-bold text-neutral-900 dark:text-white uppercase tracking-wider">Choose Size</h3>
                    </div>
                      ...
                      {resolvedProduct.sizes.map((size) => {
                        ...
                        return (
                          <button key={size.id} onClick={() => setSelectedSize(size)}
                            className={`flex flex-col items-center gap-1.5 p-1.5 md:p-2 rounded-xl transition-all duration-300 ${active ? 'bg-neutral-100 dark:bg-white/5 scale-105' : 'hover:bg-neutral-100 dark:hover:bg-white/5'}`}
                          >
                            <div className={`relative rounded-full border-2 overflow-hidden flex items-center justify-center transition-all duration-300 ${active ? 'w-12 h-12 md:w-16 md:h-16 border-yellow-500 bg-yellow-500/20 shadow-[0_0_15px_rgba(234,179,8,0.4)]' : 'w-10 h-10 md:w-14 md:h-14 border-neutral-300 dark:border-neutral-700 bg-neutral-100 dark:bg-neutral-900'}`}>
                              {size.image
                                ...
                                : <span className={`font-black text-sm md:text-base ${active ? 'text-neutral-900 dark:text-white' : 'text-neutral-600'}`}>{size.label[0] ?? '?'}</span>
                              }
                            </div>
                            <div className="text-center">
                              <div className={`text-[9px] md:text-xs font-bold uppercase ${active ? 'text-neutral-900 dark:text-white' : 'text-neutral-500'}`}>{size.label}</div>
                ...
                {activeGroups.map((group, groupIdx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span className="w-5 h-5 md:w-6 md:h-6 rounded-full bg-yellow-500 text-neutral-900 dark:text-white flex items-center justify-center font-bold text-[10px] md:text-xs">{stepNum}</span>
                        <h3 className="text-base md:text-lg font-bold text-neutral-900 dark:text-white uppercase tracking-wider">{group.name}</h3>
                        ...
                        <ChevronRight
                          size={16}
                          className={`ml-auto text-neutral-500 dark:text-neutral-400 transition-transform ${isCollapsed ? '' : 'rotate-90'}`}
                        />
                      </button>
                      ...
                      {!isCollapsed && (hasImages ? (
                          ...
                          {group.options.map(option => {
                            ...
                            return (
                              <div key={option.id} onClick={() => toggleOption(group.id, option, group.maxSelection)}
                                className={`relative aspect-square rounded-2xl border-2 cursor-pointer overflow-hidden group transition-all duration-300 ${active ? 'border-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.3)]' : 'border-neutral-300 dark:border-neutral-800 hover:border-neutral-500 dark:hover:border-neutral-600'}`}
                              >
                                <div className="absolute inset-0 bg-neutral-100 dark:bg-[#121212]">
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-white/95 dark:from-black via-white/70 dark:via-black/80 to-transparent">
                                  <div className={`font-bold text-[10px] md:text-xs text-center leading-tight ${active ? 'text-neutral-900 dark:text-white' : 'text-neutral-700 dark:text-neutral-300'}`}>{option.name}</div>
                        ...
                        <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                          {group.options.map(option => {
                            ...
                            return (
                              <div key={option.id} onClick={() => toggleOption(group.id, option, group.maxSelection)}
                                className={`relative aspect-square rounded-2xl border-2 cursor-pointer overflow-hidden flex flex-col items-center justify-center p-2 transition-all duration-300 ${active ? 'border-yellow-500 bg-yellow-500/10' : 'border-neutral-300 dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-900/50 hover:bg-neutral-200 dark:hover:bg-neutral-800'}`}
                              >
                                {active && (
                                  <div className="absolute top-2 right-2 w-5 h-5 bg-yellow-500 rounded-full flex items-center justify-center shadow-lg z-10 animate-in zoom-in duration-300">
                                    <Check size={12} className="text-black" strokeWidth={4} />
                                ...
                                <div className={`font-bold text-[10px] md:text-xs text-center leading-tight mb-0.5 ${active ? 'text-neutral-900 dark:text-white' : 'text-neutral-500 dark:text-neutral-400'}`}>{option.name}</div>
          ...
          <div className="w-full bg-white dark:bg-[#0a0a0a] border-t border-neutral-200 dark:border-white/10 p-4 md:p-6 shrink-0 z-50 shadow-[0_-5px_20px_rgba(148,163,184,0.25)] dark:shadow-[0_-5px_20px_rgba(0,0,0,0.5)]">
            <div className="flex gap-3 md:gap-4 items-center">
              <div className="flex items-center gap-2 md:gap-4 bg-neutral-100 dark:bg-[#141414] rounded-xl md:rounded-2xl p-1 md:p-1.5 border border-neutral-200 dark:border-white/5">
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white hover:bg-neutral-200 dark:hover:bg-neutral-800 rounded-lg md:rounded-xl transition-colors">
                  <Minus size={18} className="md:w-5 md:h-5" />
                </button>
                <span className="text-lg md:text-xl font-black text-neutral-900 dark:text-white w-6 md:w-8 text-center">{quantity}</span>
                <button onClick={() => setQuantity(quantity + 1)} className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white hover:bg-neutral-200 dark:hover:bg-neutral-800 rounded-lg md:rounded-xl transition-colors">
              ...
              <button
                ...
                className="flex-1 h-[52px] md:h-[60px] bg-yellow-500 hover:bg-yellow-400 text-neutral-900 dark:text-white rounded-xl md:rounded-2xl font-black text-base md:text-lg flex items-center justify-between px-4 md:px-8 shadow-[0_0_20px_rgba(234,179,8,0.3)] hover:shadow-[0_0_30px_rgba(234,179,8,0.5)] transition-all transform hover:scale-[1.02] active:scale-95"
              >
